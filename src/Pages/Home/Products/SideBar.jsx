import React from "react";
import {
  PiGridFourFill,
  PiDeviceMobileCameraFill,
  PiHeadphonesFill,
  PiDressFill,
  PiBowlFoodFill,
  PiWatchFill,
} from "react-icons/pi";

const categories = [
  { icon: <PiGridFourFill />, name: "All Categories" },
  { icon: <PiDeviceMobileCameraFill />, name: "Electronic Devices" },
  { icon: <PiHeadphonesFill />, name: "Accessories" },
  { icon: <PiDressFill />, name: "Fashion & Clothing" },
  { icon: <PiBowlFoodFill />, name: "Groceries & Foods" },
  { icon: <PiWatchFill />, name: "Watches" },
];

const SideBar = () => {
  return (
    <div className="w-full md:w-[20%] border rounded-xl border-gray-200 shadow-md p-5 bg-white sticky top-5 h-fit">

      {/* Sidebar Title */}
      <h1 className="text-xl font-semibold mb-4 border-b pb-2">
        Categories
      </h1>

      {/* Category List */}
      <div className="flex flex-col gap-2 mb-6">
        {categories.map((item, index) => (
          <div
            key={index}
            className="flex items-center gap-3 py-2 px-3 rounded-lg cursor-pointer
            hover:bg-green-100/50 group transition-all duration-300"
          >
            <span className="text-xl text-green-600 group-hover:scale-110 transition-transform">
              {item.icon}
            </span>
            <p className="text-sm font-medium text-gray-700 group-hover:text-green-700">
              {item.name}
            </p>
          </div>
        ))}
      </div>

      {/* Filters */}
      <h2 className="text-lg font-semibold text-green-700 mb-3">Filters</h2>

      {/* Price Range */}
      <div className="mb-5">
        <h3 className="font-medium text-gray-700 mb-1 text-sm">Price Range</h3>
        <div className="flex gap-2">
          <input
            type="number"
            placeholder="Min"
            className="w-1/2 border border-gray-300 rounded-lg px-2 py-1 text-sm focus:ring-1 focus:ring-green-500"
          />
          <input
            type="number"
            placeholder="Max"
            className="w-1/2 border border-gray-300 rounded-lg px-2 py-1 text-sm focus:ring-1 focus:ring-green-500"
          />
        </div>
      </div>

      {/* Brand Filter */}
      <div className="mb-5">
        <h3 className="font-medium text-gray-700 mb-1 text-sm">Brand</h3>
        <select className="w-full border border-gray-300 rounded-lg px-2 py-2 text-sm focus:ring-1 focus:ring-green-500">
          <option>All</option>
          <option>Samsung</option>
          <option>Apple</option>
          <option>Xiaomi</option>
          <option>Oppo</option>
          <option>Realme</option>
        </select>
      </div>

      {/* Rating Filter */}
      <div>
        <h3 className="font-medium text-gray-700 mb-1 text-sm">Rating</h3>
        <div className="flex flex-col gap-1 text-sm text-gray-600">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" /> 5 Stars
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" /> 4 Stars & Up
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" /> 3 Stars & Up
          </label>
        </div>
      </div>

    </div>
  );
};

export default SideBar;
